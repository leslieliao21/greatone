$(function() {
  ...
  function _headerCallback() {
    ...
    var enFiles = ['tnf100_2019', 'tnf100_2019_review', 'tnf100_2019_rule', 'tnf100_2019_info'];
    ...
    if ($('.enPage').length > 0) {
      ...
      if (!hasEnPage) {
        $enLink.attr('href', '../' + enFiles[2] + '_en.html');
      } else {
  ...
  function slickInit(slideNum) {
    if ($('.activetiesTemplate').length > 0) {
      var slickProp = {
        ...
        arrows: true,
        prevArrow:
          '<button class="slick-arrow arrow-prev inTheEnd"><div></div></>',
