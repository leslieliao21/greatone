...
switch ($etype){
    case 'sort':{
        ...
        $query = 'SELECT * FROM unit WHERE id='.$id1;
        ...
        $query = 'SELECT * FROM unit WHERE id='.$id2;
        $result = $DB->select($query,false,$param_str,$param_ary);
        ...
        if($sort1 && $sort2){
            ...
            $param_str = 'i';
            $result = updateRecord('unit','id='.$id1,$edit_ary,$param_ary,$param_str);
            
            $edit_ary = array('sort=?');
            $param_ary = array($sort1);
            $param_str = 'i';
            $result = updateRecord('unit','id='.$id2,$edit_ary,$param_ary,$param_str);
    ...
    case 'edit':{
        ...
        if($id<=0){
            ...
            $param_str = 's';
            $result = insertRecord('unit',$edit_ary,$param_ary,$param_str);
            $id = $result['id'];
            
            $edit_ary = array('sort=?');
            $param_ary = array($id);
            $param_str = 'i';
            $result = updateRecord('unit','id='.$id,$edit_ary,$param_ary,$param_str);
        }
        else{
    ...
    case 'del':{
        $id = isset($_POST['id'])?FUNCS::inputFilter('id','POST',1):0;
        $result = deleteRecord('subunit','uid='.$id);
        $result = deleteRecord('unit','id='.$id);
