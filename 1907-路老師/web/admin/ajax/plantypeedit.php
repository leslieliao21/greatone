...
switch ($etype){
    case 'sort':{
        ...
        $query = 'SELECT * FROM plantype WHERE id='.$id1;
        ...
        $query = 'SELECT * FROM plantype WHERE id='.$id2;
        $result = $DB->select($query,false,$param_str,$param_ary);
        ...
        if($sort1 && $sort2){
            ...
            $param_str = 'i';
            $result = updateRecord('plantype','id='.$id1,$edit_ary,$param_ary,$param_str);
            
            $edit_ary = array('sort=?');
            $param_ary = array($sort1);
            $param_str = 'i';
            $result = updateRecord('plantype','id='.$id2,$edit_ary,$param_ary,$param_str);
    ...
    case 'edit':{
        ...
        if($id<=0){
            ...
            $param_str = 's';
            $result = insertRecord('plantype',$edit_ary,$param_ary,$param_str);
            $id = $result['id'];
            
            $edit_ary = array('sort=?');
            $param_ary = array($id);
            $param_str = 'i';
            $result = updateRecord('plantype','id='.$id,$edit_ary,$param_ary,$param_str);
        }
        else{
    ...
    case 'del':{
        $id = isset($_POST['id'])?FUNCS::inputFilter('id','POST',1):0;
        $result = deleteRecord('plantype','id='.$id);
